# 🔌 Kết nối Graphics & HCI Portal với Google Sheets + Google Forms

> ## ✅ ĐÃ TRIỂN KHAI XONG — 14/08/2026
>
> Hệ thống đã chạy thật trên tài khoản **teacher@example.edu**. Không cần làm lại các bước bên dưới;
...
> | 🔗 **URL Web App (API)** | `https://script.google.com/macros/s/YOUR_DEPLOYMENT_ID/exec` |
>
...
Tài khoản chứa toàn bộ dữ liệu: **teacher@example.edu**
Thư mục Drive đích: **`danhgia`** → <https://drive.google.com/drive/folders/YOUR_DRIVE_FOLDER_ID>
...
1. Đăng nhập Google bằng **teacher@example.edu** (kiểm tra góc phải trên: đúng tài khoản mới làm tiếp).
2. Mở <https://script.google.com> → **Dự án mới / New project**.
3. Đặt tên dự án: `Graphics & HCI Portal API`.
...
2. Google hỏi quyền → **Xem lại quyền** → chọn tài khoản `teacher@example.edu` →
   *"Google chưa xác minh ứng dụng này"* → **Nâng cao** → **Chuyển đến Graphics & HCI Portal API (không an toàn)** → **Cho phép**.
...
3. Điền:
   - **Mô tả:** `STEM API v1`
   - **Thực thi với tư cách / Execute as:** **Tôi (teacher@example.edu)**
