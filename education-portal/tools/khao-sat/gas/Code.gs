/* =====================================================================
   ...
   Tài khoản triển khai: teacher@example.edu
   ---------------------------------------------------------------------
...
/* ───────────────────────── CẤU HÌNH ───────────────────────── */
var CONFIG = {
  OWNER_EMAIL   : 'teacher@example.edu',
  // Thư mục Drive "danhgia" của teacher@example.edu
  ...
  BASE_URL      : 'https://YOUR-ORG.github.io/YOUR-REPO/tools/khao-sat/',
  ROSTER_URL    : 'https://YOUR-ORG.github.io/YOUR-REPO/tools/khao-sat/data/students.json',
  COURSES_URL   : 'https://YOUR-ORG.github.io/YOUR-REPO/tools/khao-sat/data/courses.json'
};
...
function taoCacForm_(ss, folder) {
  ...
  var moiKhoa = khoa.length ? khoa.map(function (r) { return r[0] + ' — ' + r[2] + ' ' + r[1]; })
                            : ['ui-ux-design-10weeks — 🤖 STEM micro:bit'];
...
function dungForm_(propKey, title, desc, build, ss, folder, props) {
  ...
  form.setTitle(title).setDescription(desc)
      ...
      .setConfirmationMessage('✅ Đã ghi nhận. Cảm ơn em! — Graphics & HCI Portal');
  try {
...
function luuKhaoSat_(p, nguon, thietBi) {
  var cid = p.courseId || 'ui-ux-design-10weeks';
...
function doGet(e) {
  ...
  try {
    switch (action) {
      case 'ping':      kq = { ok: true, service: 'Graphics & HCI Portal API v2', time: new Date().toISOString(), config: thongTinCauHinh_() }; break;
...
function guiBaoCaoTongHop(email, courseId) {
  ...
  var html = [
    ...
    '<h2 style="margin:0 0 4px">📊 Báo cáo tổng hợp — Graphics & HCI Portal</h2>',
  ...
  html.push('<p style="margin-top:22px;font-size:12px;color:#6a7770">',
    ...
    'Email tự động từ Graphics & HCI Portal — Apps Script (' + CONFIG.OWNER_EMAIL + ')</p></div>');

  var body = html.join('');
  MailApp.sendEmail({
    to: email,
    subject: '📊 Báo cáo Graphics & HCI Portal — ' + ngay,
    htmlBody: body,
    name: 'Graphics & HCI Portal'
  });
